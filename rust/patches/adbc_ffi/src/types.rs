...
unsafe extern "C" fn release_ffi_error(error: *mut FFI_AdbcError) {
    match error.as_mut() {
        None => (),
        Some(error) => {
            // Free message and null the pointer to prevent double-free.
            if !error.message.is_null() {
                // SAFETY: `error.message` was obtained with `CString::into_raw`.
                drop(CString::from_raw(error.message));
                error.message = null_mut();
            }

            if !error.private_data.is_null() {
                // SAFETY: `error.private_data` was obtained with `Box::into_raw`.
                let private_data = Box::from_raw(error.private_data as *mut ErrorPrivateData);
                drop(private_data);
                error.private_data = null_mut();
            }

            // Clear the release callback so a second call is a no-op.
            error.release = None;
        }
    }
}
